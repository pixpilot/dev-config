...
import { createNodejsEslintConfig } from '../src/create-nodejs-eslint-config';

describe('createNodejsEslintConfig', () => {
  it('should be defined', () => {
    expect(createNodejsEslintConfig).toBeDefined();
  });

  it('should return a config array with all required configs', () => {
    const config = createNodejsEslintConfig();
    expect(Array.isArray(config)).toBe(true);
    // Should include base, jest, and prettier configs
    expect(config.length).toBeGreaterThanOrEqual(3);
  });

  it('should merge additional configs', () => {
    const additional = [{ name: 'custom' }];
    const config = createNodejsEslintConfig(additional);
  ...
  it('should include ignore config if options.ignoreFilePath is provided', () => {
    ...
    const config = createNodejsEslintConfig([], { ignoreFilePath: absPath });
